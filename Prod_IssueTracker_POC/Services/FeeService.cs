...
namespace Prod_IssueTracker_POC.Services
{
    ...
    {
        Task<bool> ValidateRequest(FeeRequest request, string referenceNumber, string attemptId);
        Task<Agent?> GetAgent(string agentId, string referenceNumber, string attemptId);
        Task<Branch?> GetBranch(string branchId, string referenceNumber, string attemptId);
        Task<FeeDetail?> GetFee(decimal amount, Agent agent, Branch branch, string referenceNumber, string attemptId);
        Task<FeeResponse> CalculateFee(FeeRequest request, string attemptId);
    }
    ...
    public class FeeService : IFeeService
    {
        ...
        public async Task<bool> ValidateRequest(FeeRequest request, string referenceNumber, string attemptId)
        {
            ...
            if (request == null)
            {
                ...
                _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "ValidationFailed", new { reason = "Request is null" });
                return false;
            }
            ...
            if (string.IsNullOrWhiteSpace(request.AgentId))
            {
                ...
                _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "ValidationFailed", new { reason = "AgentId is required" });
                return false;
            }
            ...
            if (string.IsNullOrWhiteSpace(request.BranchId))
            {
                ...
                _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "ValidationFailed", new { reason = "BranchId is required" });
                return false;
            }
            ...
            if (request.Amount == null || request.Amount <= 0)
            {
                ...
                _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "ValidationFailed", new { reason = "Amount must be greater than zero" });
                return false;
            }

            _logger.LogInformation("[{ReferenceNumber}] Request validation successful", referenceNumber);
            _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "ValidationSuccess");
            return await Task.FromResult(true);
        }

        public async Task<Agent?> GetAgent(string agentId, string referenceNumber, string attemptId)
        {
            ...
            if (agent == null || !agent.IsActive)
                _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "AgentInvalid");
            else
                _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "AgentRetrieved", new { agentId = agent.AgentId });

            return await Task.FromResult(agent);
        }

        public async Task<Branch?> GetBranch(string branchId, string referenceNumber, string attemptId)
        {
            ...
            if (branch == null || !branch.IsActive)
                _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "BranchInvalid");
            else
                _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "BranchRetrieved", new { branchId = branch.BranchId });

            return await Task.FromResult(branch);
        }

        public async Task<FeeDetail?> GetFee(decimal amount, Agent agent, Branch branch, string referenceNumber, string attemptId)
        {
            ...
            if (feeDetail == null)
                _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "FeeCalculationFailed");
            else
                _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "FeeCalculated", new { totalFee = feeDetail.TotalFee });

            return await Task.FromResult(feeDetail);
        }

        public async Task<FeeResponse> CalculateFee(FeeRequest request, string attemptId)
        {
            ...
            _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "RequestReceived");
            ...
            var isValid = await ValidateRequest(request, referenceNumber, attemptId);
            if (!isValid)
            {
            ...
            var agent = await GetAgent(request.AgentId!, referenceNumber, attemptId);
            if (agent == null || !agent.IsActive)
            {
                response.Success = false;
            ...
            var branch = await GetBranch(request.BranchId!, referenceNumber, attemptId);
            if (branch == null || !branch.IsActive)
            {
                response.Success = false;
            ...
            var fee = await GetFee(request.Amount!.Value, agent, branch, referenceNumber, attemptId);
            if (fee == null)
            {
                response.Success = false;
            ...
            _flowTagger.Tag(attemptId, referenceNumber, FlowMaps.GetFeeTransaction, "ProcessCompleted");
            return response;
        }
    }
}
